...
def recruiter():
    ...
    return render_template("recruiter.html", all_user=rec_user_data, data=getinfo, recruiter=recruiter)
...
def recruiter_user_page(userid):
    ...
    return render_template("rec_user_disp.html", showuser=getuserid, data=getinfo, showresult=getPersonalList, ouputRoles=getRoles)
...
# Job description code starts here
    ...
    #     print("printing the temp variable", temp.quote)
    ...
    return render_template("/tenth/resultpage.html", roles=roles, mylist=my_list, getinfo=getinfo)
...
@ app.route('/music_teacher')
def Music_Teacher_page():
    return render_template("/tenth/music_teacher.html", com=int_company, tit=int_title, loc=int_loc, link=int_link)
...
@ app.route('/test1-up', methods=['GET', 'POST'])
def test1_up():
    return render_template("/ug-pg/be/test1.html")


@ app.route('/test2-up', methods=['GET', 'POST'])
def test2_up():
    return render_template("/ug-pg/be/test2page.html")


@ app.route('/test3-up', methods=['GET', 'POST'])
def take3_up():
    return render_template("/ug-pg/be/test3page.html")


@ app.route('/results-up', methods=['GET', 'POST'])
def results_up():
    return render_template("/ug-pg/be/resultpage.html")
...
def addpost():
    ...
    adddata = User(fname=name, lname=lname, email=email, dob=date_time_str, state=state, age=age,
                   phone=phone, city=city, uname=uname, password=password1, re_password=password2, gender=gender, quali=account, fam_income=income, roles=roles)
